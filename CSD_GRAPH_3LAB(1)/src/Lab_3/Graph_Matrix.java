...
public class Graph_Matrix {
    ...
    void setAdjMatrix(int[][] m) {
        ...
        for(int i = 0; i < nVertices; i++)
            for(int j = 0; j < nVertices; j++) adjMatrix[i][j] = m[i][j];
    }
    ...
    void DFS(int v, boolean[] visited) {
        ...
        for(int j = 0; j < nVertices; j++)
            if(!visited[j] && adjMatrix[v][j] > 0) DFS(j, visited);
    }
    ...
    void DFS_F(RandomAccessFile rf, int v, boolean[] visited) throws Exception{
        ...
        visited[v] = true;
        for(int j = 0; j < nVertices; j++)
            if(!visited[j] && adjMatrix[v][j] > 0) DFS_F(rf, j, visited);
    }
    ...
    void DF_traverseAll_F(String filename) throws Exception{
        ...
        int i;
        for(i = 0; i < nVertices; i++) visited[i] = false;
        for(i = 0; i < nVertices; i++)
